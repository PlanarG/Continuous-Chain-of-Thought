...
class Block(nn.Module):
    def __init__(self, d_model, nhead, drop, maxlen, rpe, rpe_type, no_causal_mask):
        ...
        self.attn = CausalSelfAttention(
            d_model=d_model, nhead=nhead, drop=drop, maxlen=maxlen, rpe=rpe, rpe_type=rpe_type, no_causal_mask=no_causal_mask
        )
...
class CCOT(nn.Module):
    def __init__(self, args, logger):
        ...
        self.encoder_embed = nn.Embedding(config.vocab_size, config.encoder_hidden_size, self.padding_idx)
        self.decoder_embed = nn.Embedding(config.vocab_size, config.decoder_hidden_size, self.padding_idx)
    ...
    def forward(
        ...
        mask = torch.tril(torch.ones(seq_len, seq_len)).view(1, 1, seq_len, seq_len).to(hidden_states.device)
        ## TODO
        
        logits = self.lm_head(self.decoder(hidden_states, mask=mask))
        return logits
...
class GPT(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.loop_bias = args.loop_bias
        self.no_loop = args.no_loop
        self.transformer = nn.ModuleDict(dict(
            embedding=Embedding(d_model=args.dmodel, vocab_size=args.vocab),
    ...
    def forward(
        ...
        bz, seq_len = idx.size()
        emb = self.transformer.embedding(idx)  # Shape: (btsz, seq_len, emb_dim)
        x = self.transformer.drop(emb)

        original_input = x

        loop_nums = loop_nums + self.loop_bias
        loop_nums = torch.clamp(loop_nums, min=1)

        max_iterations = torch.max(loop_nums).item()
        if self.no_loop:
            max_iterations = 1
        
        pad_mask = None
        if z is not None:
            pad_mask = torch.arange(seq_len).unsqueeze(0).unsqueeze(0).to(z.device) <= z.unsqueeze(1).unsqueeze(2)
            pad_mask = pad_mask.unsqueeze(1).int()
        else:
            pad_mask = None

        for i in range(max_iterations):
            iter_mask = (loop_nums > i).float().view(-1, 1, 1)
            x = self.transformer.loop(x + original_input, mask=pad_mask) * \
                iter_mask + x * (1 - iter_mask)

        final_logits = self.lm_head(x)  # Shape: (btsz, seq_len, vocab_size)

        return final_logits

    def generate(self, idx, start, loop_nums, tempreture=None):
        b, t = idx.size()
        tmp_start = start + 0
        idx = idx.int()
        while True:
            logits = self.forward(idx, loop_nums)
            if tempreture is not None:
                dmodel = logits.size(-1)
                idx_new = torch.multinomial(F.softmax(logits.view(-1, dmodel) / tempreture, dim=-1), 1).view(b, -1).int()
            else:
                idx_new = torch.argmax(logits, dim=2).int()
            idx[torch.arange(b), tmp_start + 1] = idx_new[torch.arange(b), tmp_start]       # +1 keep the original seperator (e.g. '=')
            if (torch.sum(idx_new[torch.arange(b), tmp_start] != 2) == 0) or (torch.sum(tmp_start == t - 2) != 0):  # all eos(2) or meet maxlen
                break
